...
**Бизнес-задача:** В офисной работе существуют сценарии, где сотрудники регулярно копируют табличные данные из Excel для пересылки, согласования или вставки в другие системы, и в этих данных могут присутствовать чувствительные числовые значения, которые нужно маскировать без ручной обработки.

**Задача проекта:** Показать, как с помощью Cursor можно последовательно спроектировать и реализовать небольшой проект, который решает эту задачу через фоновую Python-утилиту: она отслеживает табличный текст в буфере обмена, распознаёт числа и подменяет их на безопасные значения, сохраняя привычный пользовательский сценарий работы.
